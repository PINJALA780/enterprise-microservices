...
The README uses repository-relative links, so once the images are committed to GitHub, they can be opened directly from the project documentation.
...
After committing the screenshots, you can also display important evidence directly in the README:
...
![Argo CD Self-Healing](docs/screenshots/argocd-self-healing.png)

## KEDA Scale-Up

![KEDA Scale-Up](docs/screenshots/keda-scale-up.png)

## End-to-End Test

![End-to-End Test](docs/screenshots/end-to-end-test.png)

Important: The links above intentionally point to the exact filenames expected in docs/screenshots/. Add your real screenshots using these names before pushing the README.
...
The current implementation is complete with Prometheus/Grafana monitoring, KEDA autoscaling, Kyverno policy enforcement, and GitOps-based deployment. The following enhancements are planned for the next phase after AWS account access is restored.

📊 Datadog Observability

Integrate Datadog as an advanced observability platform for the EKS environment.

Planned improvements:

Deploy the Datadog Agent to the EKS cluster

Collect Kubernetes, node, container, and application metrics

Centralize application and Kubernetes logs

Add distributed tracing/APM for the Spring Boot microservices

Create service-level dashboards for latency, traffic, errors, CPU, and memory

Configure monitors and alerting for production incidents

Define SLO/SLI-based observability for critical services

Prometheus and Grafana remain part of the completed project; Datadog is a future observability enhancement, not a currently implemented component.

🔐 HashiCorp Vault Secret Management

Integrate HashiCorp Vault for centralized and secure secret management.

Planned improvements:

Store database credentials and application secrets in Vault

Remove sensitive configuration from Kubernetes manifests

Integrate Vault with Kubernetes authentication

Inject secrets into application workloads securely

Implement secret rotation and short-lived credentials where appropriate

Apply least-privilege access policies

Integrate Vault with the GitOps deployment workflow without storing secrets in Git

HashiCorp Vault is a future security enhancement and is not currently part of the deployed platform.
